#!/usr/bin/env python3
"""
Fused Rotation + MXFP4 Quant — Gluon unified kernel with shuffle — Unified Gluon kernel for all M
...
def _fused_rot_quant_unified(
    ...
    # ======== Layouts ========
    TILES_N: gl.constexpr = RS // 32  # RS / (instr_N=16 * warps_N=2)
    mfma_layout: gl.constexpr = gl.amd.AMDMFMALayout(
        version=4, instr_shape=[16, 16], transposed=True,
        warps_per_cta=[2, 2], tiles_per_warp=[1, TILES_N],
    )
    ...
    FP32_EXP_BITS: gl.constexpr = 23          # mantissa bits in fp32
    FP32_EXP_MASK: gl.constexpr = 0xFF        # 8-bit exponent mask
    FP32_ROUND_UP: gl.constexpr = 0x3FFFFF    # round-up mantissa to next power-of-2
    FP32_EXP_ONLY: gl.constexpr = 0xFF800000  # mask to keep only sign+exponent
    E8M0_MIN_EXP: gl.constexpr = 2            # minimum e8m0 exponent (avoid denormals)

    acc_g = gl.reshape(acc, (BLOCK_M, NUM_QG, QG))
    amax = gl.max(gl.abs(acc_g), axis=-1)

    # Extract e8m0 scale: round amax up to nearest power-of-2, then extract exponent
    amax_u32 = amax.to(gl.uint32, bitcast=True)
    amax_u32 = (amax_u32 + FP32_ROUND_UP) & FP32_EXP_ONLY
    raw_exp = (amax_u32 >> FP32_EXP_BITS) & FP32_EXP_MASK
    e8m0 = gl.maximum(raw_exp, E8M0_MIN_EXP) - E8M0_MIN_EXP
    e8m0_u8 = e8m0.to(gl.uint8)

    # Reconstruct scale as float32: 2^e8m0
    hw_scale = (e8m0.to(gl.uint32) << FP32_EXP_BITS).to(gl.float32, bitcast=True)
    ...
    if SHUFFLE_SCALES:
        # Fused e8m0_shuffle: view(sm//32, 2, 16, sn//8, 2, 4).permute(0, 3, 5, 2, 4, 1)
        # Computes shuffled output indices directly from (m_local, rot_idx, q)
        HALF_BLOCK_M: gl.constexpr = BLOCK_M // 2    # 16: split rows into 2 groups
        SCALE_COL_GROUP: gl.constexpr = 8             # columns grouped by 8
        SCALE_COL_SUBGROUP: gl.constexpr = 4          # subgroup of 4 within group of 8
        # Shuffle strides: derived from permute(0, 3, 5, 2, 4, 1) on [1, 2, 16, sn//8, 2, 4]
        SHUF_STRIDE_I3: gl.constexpr = SCALE_COL_SUBGROUP * HALF_BLOCK_M * 2 * 2  # 256
        SHUF_STRIDE_I5: gl.constexpr = HALF_BLOCK_M * 2 * 2                        # 64
        SHUF_STRIDE_I2: gl.constexpr = 2 * 2                                        # 4
        SHUF_STRIDE_I4: gl.constexpr = 2                                             # 2

        sc_m_local = gl.arange(0, BLOCK_M, layout=gl.SliceLayout(1, sc_store))
        sc_q = gl.arange(0, NUM_QG, layout=gl.SliceLayout(0, sc_store))
        i1 = sc_m_local // HALF_BLOCK_M
        i2 = sc_m_local % HALF_BLOCK_M
        orig_col = pid_rot * NUM_QG + sc_q
        i3 = orig_col // SCALE_COL_GROUP
        i4 = (orig_col % SCALE_COL_GROUP) // SCALE_COL_SUBGROUP
        i5 = orig_col % SCALE_COL_SUBGROUP
        flat_idx = i3[None, :] * SHUF_STRIDE_I3 + i5[None, :] * SHUF_STRIDE_I5 + i2[:, None] * SHUF_STRIDE_I2 + i4[None, :] * SHUF_STRIDE_I4 + i1[:, None]
...
MXFP4_QUANT_GROUP_SIZE = 32   # elements per quantization group
SHUFFLE_COL_ALIGN = 8         # scale column alignment for e8m0_shuffle
SHUFFLE_ROW_ALIGN = 256       # scale row alignment for e8m0_shuffle


def fused_rotation_quant(x, rotation, rotation_size=128, fp4_out=None, scales_out=None, shuffle_scales=False):
    """
    ...
    n_scales = K // MXFP4_QUANT_GROUP_SIZE

    if fp4_out is None:
        fp4_out = torch.empty((M, K // 2), dtype=torch.uint8, device=x.device)

    if shuffle_scales:
        sn_padded = (n_scales + SHUFFLE_COL_ALIGN - 1) // SHUFFLE_COL_ALIGN * SHUFFLE_COL_ALIGN
        sm_padded = (M + SHUFFLE_ROW_ALIGN - 1) // SHUFFLE_ROW_ALIGN * SHUFFLE_ROW_ALIGN
    ...
    BLOCK_M = 32
    NUM_WARPS = 4
    grid = (triton.cdiv(M, BLOCK_M), K // rotation_size)
    _fused_rot_quant_unified[grid](
        ...
        RS=rotation_size, QG=MXFP4_QUANT_GROUP_SIZE, BLOCK_M=BLOCK_M,
...
if __name__ == "__main__":
    ...
    for M, K in [(1, 3584), (4, 3584), (32, 3584), (128, 3584), (32, 18944)]:
        ...
        fp4_v16, sc_v16 = fused_rotation_quant(x, rot, RS, shuffle_scales=False)
    ...
    for M, K in [(32, 3584), (64, 3584), (128, 3584), (32, 18944)]:
        ...
        # v16 fused shuffle
        fp4_v16, sc_v16 = fused_rotation_quant(x, rot, RS, shuffle_scales=True)
    ...
    for M, K in [(1, 3584), (4, 3584), (16, 3584), (32, 3584), (64, 3584), (128, 3584), (256, 3584), (32, 18944), (128, 18944)]:
        ...
        if M < 32:
            ...
            t_v16 = bench(fused_rotation_quant, x, rot, RS)
            t_v16p = bench(fused_rotation_quant, x, rot, RS, fp4, sc, False)
            note = "no shuffle"
        else:
            ...
            t_v16 = bench(fused_rotation_quant, x, rot, RS, None, None, True)
            t_v16p = bench(fused_rotation_quant, x, rot, RS, fp4, sc_pad, True)
            note = "fused shuffle"
    ...
    for M in [1, 32, 128]:
        ...
        for K in [3584, 3584, 18944, 3584]:
            ...
            if M < 32:
                ...
                fp4 = torch.empty(M, K // 2, dtype=torch.uint8, device=device)
                sc = torch.empty(M, K // 32, dtype=torch.uint8, device=device)
                total_v16 += bench(fused_rotation_quant, x, rot, RS, fp4, sc, False) * 36
            else:
                ...
                fp4 = torch.empty(M, K // 2, dtype=torch.uint8, device=device)
                sc_pad = torch.zeros(sm_pad, sn_pad, dtype=torch.uint8, device=device)
                total_v16 += bench(fused_rotation_quant, x, rot, RS, fp4, sc_pad, True) * 36
